...
BEGIN_MESSAGE_MAP(ScriptProperties, CPropertyPage)
	...
	ON_EN_CHANGE(IDC_SCRIPT_COMMENT, &ThisClass::OnChangeScriptComment)
	ON_EN_CHANGE(IDC_SCRIPT_NAME, &ThisClass::OnChangeScriptName)
	ON_BN_CLICKED(IDC_SCRIPT_ACTIVE, &ThisClass::OnScriptActive)
	ON_BN_CLICKED(IDC_ONE_SHOT, &ThisClass::OnOneShot)
	ON_BN_CLICKED(IDC_EASY, &ThisClass::OnEasy)
	ON_BN_CLICKED(IDC_HARD, &ThisClass::OnHard)
	ON_BN_CLICKED(IDC_NORMAL, &ThisClass::OnNormal)
	ON_BN_CLICKED(IDC_SCRIPT_SUBROUTINE, &ThisClass::OnScriptSubroutine)
	ON_BN_CLICKED(IDC_EVERY_FRAME, &ThisClass::OnEveryFrame)
	ON_BN_CLICKED(IDC_EVERY_SECOND, &ThisClass::OnEverySecond)
	ON_EN_CHANGE(IDC_SECONDS_EDIT, &ThisClass::OnChangeSecondsEdit)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
