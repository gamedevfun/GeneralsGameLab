...
BEGIN_MESSAGE_MAP(CTeamsDialog, CDialog)
	...
	ON_BN_CLICKED(IDC_NEWTEAM, &ThisClass::OnNewteam)
	ON_BN_CLICKED(IDC_DELETETEAM, &ThisClass::OnDeleteteam)
	ON_LBN_SELCHANGE(IDC_PLAYER_LIST, &ThisClass::OnSelchangePlayerList)
	ON_NOTIFY(NM_CLICK, IDC_TEAMS_LIST, &ThisClass::OnClickTeamsList)
	ON_NOTIFY(NM_DBLCLK, IDC_TEAMS_LIST, &ThisClass::OnDblclkTeamsList)
	ON_BN_CLICKED(IDC_COPYTEAM, &ThisClass::OnCopyteam)
	ON_BN_CLICKED(IDC_SelectTeamMembers, &ThisClass::OnSelectTeamMembers)
	ON_BN_CLICKED(IDC_MOVEDOWNTEAM, &ThisClass::OnMoveDownTeam)
	ON_BN_CLICKED(IDC_MOVEUPTEAM, &ThisClass::OnMoveUpTeam)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
