...
BEGIN_MESSAGE_MAP(MeshMoldOptions, COptionsPanel)
	...
	ON_BN_CLICKED(IDC_APPLY_MESH, &ThisClass::OnApplyMesh)
	ON_EN_CHANGE(IDC_SCALE_EDIT, &ThisClass::OnChangeScaleEdit)
	ON_EN_CHANGE(IDC_HEIGHT_EDIT, &ThisClass::OnChangeHeightEdit)
	ON_EN_CHANGE(IDC_ANGLE_EDIT, &ThisClass::OnChangeAngleEdit)
	ON_BN_CLICKED(IDC_RAISE, &ThisClass::OnRaise)
	ON_BN_CLICKED(IDC_RAISE_LOWER, &ThisClass::OnRaiseLower)
	ON_BN_CLICKED(IDC_LOWER, &ThisClass::OnLower)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
