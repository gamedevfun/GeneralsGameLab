...
BEGIN_MESSAGE_MAP(TeamIdentity, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_AI_RECRUITABLE, &ThisClass::OnAiRecruitable)
	ON_BN_CLICKED(IDC_AUTO_REINFORCE, &ThisClass::OnAutoReinforce)
	ON_EN_CHANGE(IDC_DESCRIPTION, &ThisClass::OnChangeDescription)
	ON_EN_CHANGE(IDC_MAX, &ThisClass::OnChangeMax)
	ON_EN_CHANGE(IDC_PRIORITY_DECREASE, &ThisClass::OnChangePriorityDecrease)
	ON_EN_CHANGE(IDC_PRIORITY_INCREASE, &ThisClass::OnChangePriorityIncrease)
	ON_CBN_SELCHANGE(IDC_PRODUCTION_CONDITION, &ThisClass::OnSelchangeProductionCondition)
	ON_EN_CHANGE(IDC_PRODUCTION_PRIORITY, &ThisClass::OnChangeProductionPriority)
	ON_CBN_SELCHANGE(IDC_HOME_WAYPOINT, &ThisClass::OnSelchangeHomeWaypoint)
	ON_BN_CLICKED(IDC_UNIT_TYPE1_BUTTON, &ThisClass::OnUnitType1Button)
	ON_BN_CLICKED(IDC_UNIT_TYPE2_BUTTON, &ThisClass::OnUnitType2Button)
	ON_BN_CLICKED(IDC_UNIT_TYPE3_BUTTON, &ThisClass::OnUnitType3Button)
	ON_BN_CLICKED(IDC_UNIT_TYPE4_BUTTON, &ThisClass::OnUnitType4Button)
	ON_BN_CLICKED(IDC_UNIT_TYPE5_BUTTON, &ThisClass::OnUnitType5Button)
	ON_BN_CLICKED(IDC_UNIT_TYPE6_BUTTON, &ThisClass::OnUnitType6Button)
	ON_BN_CLICKED(IDC_UNIT_TYPE7_BUTTON, &ThisClass::OnUnitType7Button)
	ON_BN_CLICKED(IDC_PRODUCTION_EXECUTEACTIONS, &ThisClass::OnExecuteActions)
	ON_EN_CHANGE(IDC_TEAM_NAME, &ThisClass::OnChangeTeamName)
	ON_BN_CLICKED(IDC_TEAM_SINGLETON, &ThisClass::OnTeamSingleton)
	ON_EN_KILLFOCUS(IDC_TEAM_NAME, &ThisClass::OnKillfocusTeamName)
	ON_CBN_SELENDOK(IDC_TEAMOWNER, &ThisClass::OnSelendokTeamowner)
	ON_EN_CHANGE(IDC_TEAM_BUILD_FRAMES, &ThisClass::OnChangeTeamBuildFrames)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
