...
BEGIN_MESSAGE_MAP(DebugWindowDialog, CDialog)
	ON_WM_CREATE( )
	ON_BN_CLICKED(IDC_Pause, &ThisClass::OnPause)
	ON_BN_CLICKED(IDC_Step, &ThisClass::OnStep)
	ON_BN_CLICKED(IDC_RUN_FAST, &ThisClass::OnRunFast)
	ON_BN_CLICKED(IDC_StepTen, &ThisClass::OnStepTen)
	ON_BN_CLICKED(IDC_ClearWindows, &ThisClass::OnClearWindows)
	ON_WM_CLOSE()
	ON_WM_SIZE()
END_MESSAGE_MAP()