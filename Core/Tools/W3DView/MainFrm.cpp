...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_WM_CREATE()
	ON_COMMAND(IDM_OBJECT_PROPERTIES, &ThisClass::OnObjectProperties)
	ON_UPDATE_COMMAND_UI(IDM_OBJECT_PROPERTIES, &ThisClass::OnUpdateObjectProperties)
	ON_COMMAND(IDM_LOD_GENERATE, &ThisClass::OnLodGenerate)
	ON_WM_ACTIVATEAPP()
	ON_COMMAND(ID_FILE_OPEN, &ThisClass::OnFileOpen)
	ON_COMMAND(IDM_ANI_SPEED, &ThisClass::OnAniSpeed)
	ON_COMMAND(IDM_ANI_STOP, &ThisClass::OnAniStop)
	ON_COMMAND(IDM_ANI_START, &ThisClass::OnAniStart)
	ON_COMMAND(IDM_ANI_PAUSE, &ThisClass::OnAniPause)
	ON_COMMAND(IDM_CAMERA_BACK, &ThisClass::OnCameraBack)
	ON_COMMAND(IDM_CAMERA_BOTTOM, &ThisClass::OnCameraBottom)
	ON_COMMAND(IDM_CAMERA_FRONT, &ThisClass::OnCameraFront)
	ON_COMMAND(IDM_CAMERA_LEFT, &ThisClass::OnCameraLeft)
	ON_COMMAND(IDM_CAMERA_RESET, &ThisClass::OnCameraReset)
	ON_COMMAND(IDM_CAMERA_RIGHT, &ThisClass::OnCameraRight)
	ON_COMMAND(IDM_CAMERA_TOP, &ThisClass::OnCameraTop)
	ON_COMMAND(IDM_OBJECT_ROTATE_Z, &ThisClass::OnObjectRotateZ)
	ON_COMMAND(IDM_OBJECT_ROTATE_Y, &ThisClass::OnObjectRotateY)
	ON_COMMAND(IDM_OBJECT_ROTATE_X, &ThisClass::OnObjectRotateX)
	ON_COMMAND(IDM_LIGHT_AMBIENT, &ThisClass::OnLightAmbient)
	ON_COMMAND(IDM_LIGHT_SCENE, &ThisClass::OnLightScene)
	ON_COMMAND(IDM_BACKGROUND_COLOR, &ThisClass::OnBackgroundColor)
	ON_COMMAND(IDM_BACKGROUND_BMP, &ThisClass::OnBackgroundBMP)
	ON_COMMAND(IDM_SAVE_SETTINGS, &ThisClass::OnSaveSettings)
	ON_COMMAND(IDM_LOAD_SETTINGS, &ThisClass::OnLoadSettings)
	ON_COMMAND(IDM_LOD_SET_SWITCH, &ThisClass::OnLODSetSwitch)
	ON_COMMAND(IDM_LOD_SAVE, &ThisClass::OnLODSave)
	ON_COMMAND(IDM_LOD_SAVEALL, &ThisClass::OnLODSaveAll)
	ON_COMMAND(IDM_BACKGROUND_OBJECT, &ThisClass::OnBackgroundObject)
	ON_UPDATE_COMMAND_UI(IDM_VIEW_ANIMATION_BAR, &ThisClass::OnUpdateViewAnimationBar)
	ON_UPDATE_COMMAND_UI(IDM_VIEW_OBJECT_BAR, &ThisClass::OnUpdateViewObjectBar)
	ON_COMMAND(IDM_VIEW_ANIMATION_BAR, &ThisClass::OnViewAnimationBar)
	ON_COMMAND(IDM_VIEW_OBJECT_BAR, &ThisClass::OnViewObjectBar)
	ON_COMMAND(IDM_ANI_STEP_FWD, &ThisClass::OnAniStepFwd)
	ON_COMMAND(IDM_ANI_STEP_BKWD, &ThisClass::OnAniStepBkwd)
	ON_COMMAND(IDM_OBJECT_RESET, &ThisClass::OnObjectReset)
	ON_COMMAND(IDM_CAMERA_ALLOW_ROTATE_X, &ThisClass::OnCameraAllowRotateX)
	ON_COMMAND(IDM_CAMERA_ALLOW_ROTATE_Y, &ThisClass::OnCameraAllowRotateY)
	ON_COMMAND(IDM_CAMERA_ALLOW_ROTATE_Z, &ThisClass::OnCameraAllowRotateZ)
	ON_UPDATE_COMMAND_UI(IDM_CAMERA_ALLOW_ROTATE_X, &ThisClass::OnUpdateCameraAllowRotateX)
	ON_UPDATE_COMMAND_UI(IDM_CAMERA_ALLOW_ROTATE_Y, &ThisClass::OnUpdateCameraAllowRotateY)
	ON_UPDATE_COMMAND_UI(IDM_CAMERA_ALLOW_ROTATE_Z, &ThisClass::OnUpdateCameraAllowRotateZ)
	ON_UPDATE_COMMAND_UI(IDM_OBJECT_ROTATE_X, &ThisClass::OnUpdateObjectRotateX)
	ON_UPDATE_COMMAND_UI(IDM_OBJECT_ROTATE_Y, &ThisClass::OnUpdateObjectRotateY)
	ON_UPDATE_COMMAND_UI(IDM_OBJECT_ROTATE_Z, &ThisClass::OnUpdateObjectRotateZ)
	ON_COMMAND(IDM_DEVICE_CHANGE, &ThisClass::OnDeviceChange)
	ON_COMMAND(IDM_VIEW_FULLSCREEN, &ThisClass::OnViewFullscreen)
	ON_UPDATE_COMMAND_UI(IDM_VIEW_FULLSCREEN, &ThisClass::OnUpdateViewFullscreen)
	ON_WM_WINDOWPOSCHANGING()
	ON_WM_GETMINMAXINFO()
	ON_COMMAND(IDM_CREATE_EMITTER, &ThisClass::OnCreateEmitter)
	ON_COMMAND(IDM_EDIT_EMITTER, &ThisClass::OnEditEmitter)
	ON_UPDATE_COMMAND_UI(IDM_EDIT_EMITTER, &ThisClass::OnUpdateEditEmitter)
	ON_COMMAND(IDM_SAVE_EMITTER, &ThisClass::OnSaveEmitter)
	ON_UPDATE_COMMAND_UI(IDM_SAVE_EMITTER, &ThisClass::OnUpdateSaveEmitter)
	ON_COMMAND(IDM_BONE_AUTO_ASSIGN, &ThisClass::OnBoneAutoAssign)
	ON_COMMAND(IDM_BONE_MANAGEMENT, &ThisClass::OnBoneManagement)
	ON_COMMAND(IDM_SAVE_AGGREGATE, &ThisClass::OnSaveAggregate)
	ON_COMMAND(IDM_CAMERA_ANIMATE, &ThisClass::OnCameraAnimate)
	ON_UPDATE_COMMAND_UI(IDM_CAMERA_ANIMATE, &ThisClass::OnUpdateCameraAnimate)
	ON_UPDATE_COMMAND_UI(IDM_LOD_SAVE, &ThisClass::OnUpdateLodSave)
	ON_UPDATE_COMMAND_UI(IDM_SAVE_AGGREGATE, &ThisClass::OnUpdateSaveAggregate)
	ON_COMMAND(IDM_CAMERA_RESET_ON_LOAD, &ThisClass::OnCameraResetOnLoad)
	ON_UPDATE_COMMAND_UI(IDM_CAMERA_RESET_ON_LOAD, &ThisClass::OnUpdateCameraResetOnLoad)
	ON_COMMAND(IDM_OBJECT_ROTATE_Y_BACK, &ThisClass::OnObjectRotateYBack)
	ON_COMMAND(IDM_OBJECT_ROTATE_Z_BACK, &ThisClass::OnObjectRotateZBack)
	ON_COMMAND(IDM_LIGHT_ROTATE_Y, &ThisClass::OnLightRotateY)
	ON_COMMAND(IDM_LIGHT_ROTATE_Y_BACK, &ThisClass::OnLightRotateYBack)
	ON_COMMAND(IDM_LIGHT_ROTATE_Z, &ThisClass::OnLightRotateZ)
	ON_COMMAND(IDM_LIGHT_ROTATE_Z_BACK, &ThisClass::OnLightRotateZBack)
	ON_WM_DESTROY()
	ON_COMMAND(IDM_DEC_LIGHT, &ThisClass::OnDecLight)
	ON_COMMAND(IDM_INC_LIGHT, &ThisClass::OnIncLight)
	ON_COMMAND(IDM_DEC_AMBIENT_LIGHT, &ThisClass::OnDecAmbientLight)
	ON_COMMAND(IDM_INC_AMBIENT_LIGHT, &ThisClass::OnIncAmbientLight)
	ON_COMMAND(IDM_MAKE_AGGREGATE, &ThisClass::OnMakeAggregate)
	ON_COMMAND(IDM_RENAME_AGGREGATE, &ThisClass::OnRenameAggregate)
	ON_COMMAND(IDM_CRASH_APP, &ThisClass::OnCrashApp)
	ON_COMMAND(IDM_LOD_RECORD_SCREEN_AREA, &ThisClass::OnLODRecordScreenArea)
	ON_COMMAND(IDM_LOD_INCLUDE_NULL, &ThisClass::OnLODIncludeNull)
	ON_UPDATE_COMMAND_UI(IDM_LOD_INCLUDE_NULL, &ThisClass::OnUpdateLODIncludeNull)
	ON_COMMAND(IDM_LOD_PREV_LEVEL, &ThisClass::OnLodPrevLevel)
	ON_UPDATE_COMMAND_UI(IDM_LOD_PREV_LEVEL, &ThisClass::OnUpdateLodPrevLevel)
	ON_COMMAND(IDM_LOD_NEXT_LEVEL, &ThisClass::OnLodNextLevel)
	ON_UPDATE_COMMAND_UI(IDM_LOD_NEXT_LEVEL, &ThisClass::OnUpdateLodNextLevel)
	ON_COMMAND(IDM_LOD_AUTOSWITCH, &ThisClass::OnLodAutoswitch)
	ON_UPDATE_COMMAND_UI(IDM_LOD_AUTOSWITCH, &ThisClass::OnUpdateLodAutoswitch)
	ON_UPDATE_COMMAND_UI(IDM_MAKE_MOVIE, &ThisClass::OnUpdateMakeMovie)
	ON_COMMAND(IDM_MAKE_MOVIE, &ThisClass::OnMakeMovie)
	ON_COMMAND(IDM_SAVE_SCREENSHOT, &ThisClass::OnSaveScreenshot)
	ON_COMMAND(IDM_SLIDESHOW_DOWN, &ThisClass::OnSlideshowDown)
	ON_COMMAND(IDM_SLIDESHOW_UP, &ThisClass::OnSlideshowUp)
	ON_COMMAND(IDM_ADVANCED_ANIM, &ThisClass::OnAdvancedAnim)
	ON_UPDATE_COMMAND_UI(IDM_ADVANCED_ANIM, &ThisClass::OnUpdateAdvancedAnim)
	ON_COMMAND(IDM_CAMERA_SETTINGS, &ThisClass::OnCameraSettings)
	ON_COMMAND(IDM_COPY_SCREEN_SIZE, &ThisClass::OnCopyScreenSize)
	ON_COMMAND(IDC_LIST_MISSING_TEXTURES, &ThisClass::OnListMissingTextures)
	ON_COMMAND(IDC_COPY_ASSETS, &ThisClass::OnCopyAssets)
	ON_UPDATE_COMMAND_UI(IDC_COPY_ASSETS, &ThisClass::OnUpdateCopyAssets)
	ON_COMMAND(IDM_LIGHTING_EXPOSE, &ThisClass::OnLightingExpose)
	ON_UPDATE_COMMAND_UI(IDM_LIGHTING_EXPOSE, &ThisClass::OnUpdateLightingExpose)
	ON_COMMAND(IDM_TEXTURE_PATH, &ThisClass::OnTexturePath)
	ON_COMMAND(IDM_CHANGE_RESOLUTION, &ThisClass::OnChangeResolution)
	ON_COMMAND(IDM_CREATE_SPHERE, &ThisClass::OnCreateSphere)
	ON_COMMAND(IDM_CREATE_RING, &ThisClass::OnCreateRing)
	ON_UPDATE_COMMAND_UI(IDM_EDIT_PRIMITIVE, &ThisClass::OnUpdateEditPrimitive)
	ON_COMMAND(IDM_EDIT_PRIMITIVE, &ThisClass::OnEditPrimitive)
	ON_COMMAND(IDM_EXPORT_PRIMITIVE, &ThisClass::OnExportPrimitive)
	ON_UPDATE_COMMAND_UI(IDM_EXPORT_PRIMITIVE, &ThisClass::OnUpdateExportPrimitive)
	ON_COMMAND(IDM_KILL_SCENE_LIGHT, &ThisClass::OnKillSceneLight)
	ON_COMMAND(IDM_PRELIT_MULTIPASS, &ThisClass::OnPrelitMultipass)
	ON_UPDATE_COMMAND_UI(IDM_PRELIT_MULTIPASS, &ThisClass::OnUpdatePrelitMultipass)
	ON_COMMAND(IDM_PRELIT_MULTITEX, &ThisClass::OnPrelitMultitex)
	ON_UPDATE_COMMAND_UI(IDM_PRELIT_MULTITEX, &ThisClass::OnUpdatePrelitMultitex)
	ON_COMMAND(IDM_PRELIT_VERTEX, &ThisClass::OnPrelitVertex)
	ON_UPDATE_COMMAND_UI(IDM_PRELIT_VERTEX, &ThisClass::OnUpdatePrelitVertex)
	ON_COMMAND(IDC_ADD_TO_LINEUP, &ThisClass::OnAddToLineup)
	ON_UPDATE_COMMAND_UI(IDC_ADD_TO_LINEUP, &ThisClass::OnUpdateAddToLineup)
	ON_COMMAND(IDM_IMPORT_FACIAL_ANIMS, &ThisClass::OnImportFacialAnims)
	ON_UPDATE_COMMAND_UI(IDM_IMPORT_FACIAL_ANIMS, &ThisClass::OnUpdateImportFacialAnims)
	ON_COMMAND(IDM_RESTRICT_ANIMS, &ThisClass::OnRestrictAnims)
	ON_UPDATE_COMMAND_UI(IDM_RESTRICT_ANIMS, &ThisClass::OnUpdateRestrictAnims)
	ON_COMMAND(IDM_BIND_SUBOBJECT_LOD, &ThisClass::OnBindSubobjectLod)
	ON_UPDATE_COMMAND_UI(IDM_BIND_SUBOBJECT_LOD, &ThisClass::OnUpdateBindSubobjectLod)
	ON_COMMAND(IDM_SET_CAMERA_DISTANCE, &ThisClass::OnSetCameraDistance)
	ON_COMMAND(IDM_OBJECT_ALTERNATE_MATERIALS, &ThisClass::OnObjectAlternateMaterials)
	ON_COMMAND(IDM_CREATE_SOUND_OBJECT, &ThisClass::OnCreateSoundObject)
	ON_COMMAND(IDM_EDIT_SOUND_OBJECT, &ThisClass::OnEditSoundObject)
	ON_UPDATE_COMMAND_UI(IDM_EDIT_SOUND_OBJECT, &ThisClass::OnUpdateEditSoundObject)
	ON_COMMAND(IDM_EXPORT_SOUND_OBJ, &ThisClass::OnExportSoundObj)
	ON_UPDATE_COMMAND_UI(IDM_EXPORT_SOUND_OBJ, &ThisClass::OnUpdateExportSoundObj)
	ON_COMMAND(IDM_WIREFRAME_MODE, &ThisClass::OnWireframeMode)
	ON_UPDATE_COMMAND_UI(IDM_WIREFRAME_MODE, &ThisClass::OnUpdateWireframeMode)
	ON_UPDATE_COMMAND_UI(IDM_BACKGROUND_FOG, &ThisClass::OnUpdateBackgroundFog)
	ON_COMMAND(IDM_BACKGROUND_FOG, &ThisClass::OnBackgroundFog)
	ON_UPDATE_COMMAND_UI(IDM_SCALE_EMITTER, &ThisClass::OnUpdateScaleEmitter)
	ON_COMMAND(IDM_SCALE_EMITTER, &ThisClass::OnScaleEmitter)
	ON_UPDATE_COMMAND_UI(IDM_TOGGLE_SORTING, &ThisClass::OnUpdateToggleSorting)
	ON_COMMAND(IDM_TOGGLE_SORTING, &ThisClass::OnToggleSorting)
	ON_COMMAND(IDM_CAMERA_BONE_POS_X, &ThisClass::OnCameraBonePosX)
	ON_UPDATE_COMMAND_UI(IDM_CAMERA_BONE_POS_X, &ThisClass::OnUpdateCameraBonePosX)
	ON_COMMAND(ID_VIEW_PATCH_GAP_FILL, &ThisClass::OnViewPatchGapFill)
	ON_UPDATE_COMMAND_UI(ID_VIEW_PATCH_GAP_FILL, &ThisClass::OnUpdateViewPatchGapFill)
	ON_COMMAND(ID_VIEW_SUBDIVISION_1, &ThisClass::OnViewSubdivision1)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SUBDIVISION_1, &ThisClass::OnUpdateViewSubdivision1)
	ON_COMMAND(ID_VIEW_SUBDIVISION_2, &ThisClass::OnViewSubdivision2)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SUBDIVISION_2, &ThisClass::OnUpdateViewSubdivision2)
	ON_COMMAND(ID_VIEW_SUBDIVISION_3, &ThisClass::OnViewSubdivision3)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SUBDIVISION_3, &ThisClass::OnUpdateViewSubdivision3)
	ON_COMMAND(ID_VIEW_SUBDIVISION_4, &ThisClass::OnViewSubdivision4)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SUBDIVISION_4, &ThisClass::OnUpdateViewSubdivision4)
	ON_COMMAND(ID_VIEW_SUBDIVISION_5, &ThisClass::OnViewSubdivision5)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SUBDIVISION_5, &ThisClass::OnUpdateViewSubdivision5)
	ON_COMMAND(ID_VIEW_SUBDIVISION_6, &ThisClass::OnViewSubdivision6)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SUBDIVISION_6, &ThisClass::OnUpdateViewSubdivision6)
	ON_COMMAND(ID_VIEW_SUBDIVISION_7, &ThisClass::OnViewSubdivision7)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SUBDIVISION_7, &ThisClass::OnUpdateViewSubdivision7)
	ON_COMMAND(ID_VIEW_SUBDIVISION_8, &ThisClass::OnViewSubdivision8)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SUBDIVISION_8, &ThisClass::OnUpdateViewSubdivision8)
	ON_COMMAND(IDM_MUNGE_SORT_ON_LOAD, &ThisClass::OnMungeSortOnLoad)
	ON_UPDATE_COMMAND_UI(IDM_MUNGE_SORT_ON_LOAD, &ThisClass::OnUpdateMungeSortOnLoad)
	ON_COMMAND(IDM_ENABLE_GAMMA_CORRECTION, &ThisClass::OnEnableGammaCorrection)
	ON_UPDATE_COMMAND_UI(IDM_ENABLE_GAMMA_CORRECTION, &ThisClass::OnUpdateEnableGammaCorrection)
	ON_COMMAND(IDM_SET_GAMMA, &ThisClass::OnSetGamma)
	ON_COMMAND(IDM_EDIT_ANIMATED_SOUNDS_OPTIONS, &ThisClass::OnEditAnimatedSoundsOptions)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
