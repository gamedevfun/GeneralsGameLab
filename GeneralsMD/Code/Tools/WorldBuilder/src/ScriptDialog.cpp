...
BEGIN_MESSAGE_MAP(ScriptDialog, CDialog)
	...
	ON_NOTIFY(TVN_SELCHANGED, IDC_SCRIPT_TREE, &ThisClass::OnSelchangedScriptTree)
	ON_BN_CLICKED(IDC_NEW_FOLDER, &ThisClass::OnNewFolder)
	ON_BN_CLICKED(IDC_NEW_SCRIPT, &ThisClass::OnNewScript)
	ON_BN_CLICKED(IDC_EDIT_SCRIPT, &ThisClass::OnEditScript)
	ON_BN_CLICKED(IDC_COPY_SCRIPT, &ThisClass::OnCopyScript)
	ON_BN_CLICKED(IDC_DELETE, &ThisClass::OnDelete)
	ON_BN_CLICKED(IDC_VERIFY, &ThisClass::OnVerify)
	ON_BN_CLICKED(IDC_PATCH_GC, &ThisClass::OnPatchGC)
	ON_BN_CLICKED(IDC_AUTO_VERIFY, &ThisClass::OnAutoVerify)
	ON_BN_CLICKED(IDC_SAVE, &ThisClass::OnSave)
	ON_BN_CLICKED(IDC_LOAD, &ThisClass::OnLoad)
	ON_NOTIFY(NM_DBLCLK, IDC_SCRIPT_TREE, &ThisClass::OnDblclkScriptTree)
	ON_NOTIFY(TVN_BEGINDRAG, IDC_SCRIPT_TREE, &ThisClass::OnBegindragScriptTree)
	ON_COMMAND(ID_SCRIPTACTIVATE, &ThisClass::OnScriptActivate)
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
	ON_WM_MOVE()
	//}}AFX_MSG_MAP
