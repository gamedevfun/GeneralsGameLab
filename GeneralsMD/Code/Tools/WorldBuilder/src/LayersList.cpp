...
BEGIN_MESSAGE_MAP(LayersList, CDialog)
	ON_NOTIFY(TVN_BEGINLABELEDIT, IDC_LL_TREE, &ThisClass::OnBeginEditLabel)
	ON_NOTIFY(TVN_ENDLABELEDIT, IDC_LL_TREE, &ThisClass::OnEndEditLabel)
	ON_COMMAND(ID_SELECTLAYEROBJECT, &ThisClass::OnSelectLayerObject)
	ON_COMMAND(ID_INSERTNEWLAYER, &ThisClass::OnNewLayer)
	ON_COMMAND(ID_DELETECURRENTLAYER, &ThisClass::OnDeleteLayer)
	ON_COMMAND(ID_HIDECURRENTLAYER, &ThisClass::OnHideShowLayer)
	ON_COMMAND(ID_SELECTACTIVELAYER, &ThisClass::OnSelectActiveLayer)
	ON_COMMAND_RANGE(ID_LAYERSLIST_MERGELAYERINTO_BEGIN, ID_LAYERSLIST_MERGELAYERINTO_END, &ThisClass::OnMergeLayer)
	ON_COMMAND_RANGE(ID_LAYERSLIST_MERGEOBJECTINTO_BEGIN, ID_LAYERSLIST_MERGEOBJECTINTO_END, &ThisClass::OnMergeObject)
	ON_COMMAND_RANGE(ID_LAYERSLIST_MERGEVIEWSELECTIONINTO_BEGIN, ID_LAYERSLIST_MERGEVIEWSELECTIONINTO_END, &ThisClass::OnMergeViewSelection)
END_MESSAGE_MAP()
