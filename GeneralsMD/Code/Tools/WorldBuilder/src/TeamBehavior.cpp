...
BEGIN_MESSAGE_MAP(TeamBehavior, CPropertyPage)
	...
	ON_CBN_SELCHANGE(IDC_ON_CREATE_SCRIPT, &ThisClass::OnSelchangeOnCreateScript)
	ON_BN_CLICKED(IDC_TRANSPORTS_RETURN, &ThisClass::OnTransportsReturn)
	ON_BN_CLICKED(IDC_AVOID_THREATS, &ThisClass::OnAvoidThreats)
	ON_CBN_SELCHANGE(IDC_ON_ENEMY_SIGHTED, &ThisClass::OnSelchangeOnEnemySighted)
	ON_CBN_SELCHANGE(IDC_ON_DESTROYED, &ThisClass::OnSelchangeOnDestroyed)
	ON_CBN_SELCHANGE(IDC_ON_UNIT_DESTROYED_SCRIPT, &ThisClass::OnSelchangeOnUnitDestroyed)
	ON_BN_CLICKED(IDC_PERIMETER_DEFENSE, &ThisClass::OnPerimeterDefense)
	ON_BN_CLICKED(IDC_BASE_DEFENSE, &ThisClass::OnBaseDefense)
	ON_EN_CHANGE(IDC_PERCENT_DESTROYED, &ThisClass::OnChangePercentDestroyed)
	ON_CBN_SELCHANGE(IDC_ENEMY_INTERACTIONS, &ThisClass::OnSelchangeEnemyInteractions)
	ON_CBN_SELCHANGE(IDC_ON_ALL_CLEAR, &ThisClass::OnSelchangeOnAllClear)
	ON_CBN_SELCHANGE(IDC_ON_IDLE_SCRIPT, &ThisClass::OnSelchangeOnIdleScript)
	ON_BN_CLICKED(IDC_ATTACK_COMMON_TARGET, &ThisClass::OnAttackCommonTarget)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
