...
BEGIN_MESSAGE_MAP(CWorldBuilderDoc, CDocument)
	...
	ON_COMMAND(ID_EDIT_REDO, &ThisClass::OnEditRedo)
	ON_UPDATE_COMMAND_UI(ID_EDIT_REDO, &ThisClass::OnUpdateEditRedo)
	ON_COMMAND(ID_EDIT_UNDO, &ThisClass::OnEditUndo)
	ON_UPDATE_COMMAND_UI(ID_EDIT_UNDO, &ThisClass::OnUpdateEditUndo)
	ON_COMMAND(ID_TS_INFO, &ThisClass::OnTsInfo)
	ON_COMMAND(ID_TS_CANONICAL, &ThisClass::OnTsCanonical)
	ON_UPDATE_COMMAND_UI(ID_TS_CANONICAL, &ThisClass::OnUpdateTsCanonical)
	ON_COMMAND(ID_FILE_RESIZE, &ThisClass::OnFileResize)
	ON_COMMAND(ID_FILE_JUMPTOGAME, &ThisClass::OnJumpToGame)
	ON_COMMAND(ID_TS_REMAP, &ThisClass::OnTsRemap)
	ON_COMMAND(ID_EDIT_LINK_CENTERS, &ThisClass::OnEditLinkCenters)
	ON_UPDATE_COMMAND_UI(ID_EDIT_LINK_CENTERS, &ThisClass::OnUpdateEditLinkCenters)
	ON_COMMAND(ID_VIEW_TIME_OF_DAY, &ThisClass::OnViewTimeOfDay)
	ON_COMMAND(ID_WINDOW_2DWINDOW, &ThisClass::OnWindow2dwindow)
	ON_UPDATE_COMMAND_UI(ID_WINDOW_2DWINDOW, &ThisClass::OnUpdateWindow2dwindow)
	ON_COMMAND(ID_VIEW_RELOADTEXTURES, &ThisClass::OnViewReloadtextures)
	ON_COMMAND(ID_EDIT_SCRIPTS, &ThisClass::OnEditScripts)
	ON_COMMAND(ID_SCRIPT_EDIT, &ThisClass::OnEditScripts)
	ON_COMMAND(ID_VIEWHOME, &ThisClass::OnViewHome)
	ON_COMMAND(ID_TEXTURESIZING_TILE4X4, &ThisClass::OnTexturesizingTile4x4)
	ON_UPDATE_COMMAND_UI(ID_TEXTURESIZING_TILE4X4, &ThisClass::OnUpdateTexturesizingTile4x4)
	ON_COMMAND(ID_TEXTURESIZING_TILE6X6, &ThisClass::OnTexturesizingTile6x6)
	ON_UPDATE_COMMAND_UI(ID_TEXTURESIZING_TILE6X6, &ThisClass::OnUpdateTexturesizingTile6x6)
	ON_COMMAND(ID_TEXTURESIZING_TILE8X8, &ThisClass::OnTexturesizingTile8x8)
	ON_UPDATE_COMMAND_UI(ID_TEXTURESIZING_TILE8X8, &ThisClass::OnUpdateTexturesizingTile8x8)
	ON_COMMAND(ID_FILE_DUMPTOFILE, &ThisClass::OnDumpDocToText)
	ON_COMMAND(ID_TEXTURESIZING_REMOVECLIFFTEXMAPPING, &ThisClass::OnRemoveclifftexmapping)
	ON_COMMAND(ID_TOGGLE_PITCH_AND_ROTATE, &ThisClass::OnTogglePitchAndRotation)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
