...
BEGIN_MESSAGE_MAP(WbView3d, WbView)
	...
	ON_WM_DESTROY()
	ON_WM_SHOWWINDOW()
	ON_COMMAND(ID_VIEW_SHOWWIREFRAME, &ThisClass::OnViewShowwireframe)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWWIREFRAME, &ThisClass::OnUpdateViewShowwireframe)
	ON_WM_ERASEBKGND()
	ON_COMMAND(ID_VIEW_SHOWENTIRE3DMAP, &ThisClass::OnViewShowentire3dmap)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWENTIRE3DMAP, &ThisClass::OnUpdateViewShowentire3dmap)
	ON_COMMAND(ID_VIEW_SHOWTOPDOWNVIEW, &ThisClass::OnViewShowtopdownview)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWTOPDOWNVIEW, &ThisClass::OnUpdateViewShowtopdownview)
	ON_COMMAND(ID_VIEW_SHOWCLOUDS, &ThisClass::OnViewShowclouds)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWCLOUDS, &ThisClass::OnUpdateViewShowclouds)
	ON_COMMAND(ID_VIEW_SHOWMACROTEXTURE, &ThisClass::OnViewShowmacrotexture)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWMACROTEXTURE, &ThisClass::OnUpdateViewShowmacrotexture)
	ON_COMMAND(ID_EDIT_SELECTMACROTEXTURE, &ThisClass::OnEditSelectmacrotexture)
	ON_COMMAND(ID_VIEW_SHOWSOFTWATER, &ThisClass::OnViewShowSoftWater)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWEXTRABLENDS, &ThisClass::OnUpdateViewShowExtraBlends)
	ON_COMMAND(ID_VIEW_SHOWEXTRABLENDS, &ThisClass::OnViewExtraBlends)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWSOFTWATER, &ThisClass::OnUpdateViewShowSoftWater)
	ON_COMMAND(ID_VIEW_SHOWSHADOWS, &ThisClass::OnViewShowshadows)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWSHADOWS, &ThisClass::OnUpdateViewShowshadows)
	ON_COMMAND(ID_EDIT_SHADOWS, &ThisClass::OnEditShadows)
	ON_COMMAND(ID_EDIT_MAPSETTINGS, &ThisClass::OnEditMapSettings)
	ON_COMMAND(ID_VIEW_SHOWIMPASSABLEAREAS, &ThisClass::OnViewShowimpassableareas)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWIMPASSABLEAREAS, &ThisClass::OnUpdateViewShowimpassableareas)
	ON_COMMAND(ID_VIEW_IMPASSABLEAREAOPTIONS, &ThisClass::OnImpassableAreaOptions)
	ON_COMMAND(ID_VIEW_PARTIALMAPSIZE_96X96, &ThisClass::OnViewPartialmapsize96x96)
	ON_UPDATE_COMMAND_UI(ID_VIEW_PARTIALMAPSIZE_96X96, &ThisClass::OnUpdateViewPartialmapsize96x96)
	ON_COMMAND(ID_VIEW_PARTIALMAPSIZE_192X192, &ThisClass::OnViewPartialmapsize192x192)
	ON_UPDATE_COMMAND_UI(ID_VIEW_PARTIALMAPSIZE_192X192, &ThisClass::OnUpdateViewPartialmapsize192x192)
	ON_COMMAND(ID_VIEW_PARTIALMAPSIZE_160X160, &ThisClass::OnViewPartialmapsize160x160)
	ON_UPDATE_COMMAND_UI(ID_VIEW_PARTIALMAPSIZE_160X160, &ThisClass::OnUpdateViewPartialmapsize160x160)
	ON_COMMAND(ID_VIEW_PARTIALMAPSIZE_128X128, &ThisClass::OnViewPartialmapsize128x128)
	ON_UPDATE_COMMAND_UI(ID_VIEW_PARTIALMAPSIZE_128X128, &ThisClass::OnUpdateViewPartialmapsize128x128)
	ON_COMMAND(ID_VIEW_SHOWMODELS, &ThisClass::OnViewShowModels)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWMODELS, &ThisClass::OnUpdateViewShowModels)
	ON_COMMAND(ID_VIEW_BOUNDINGBOXES, &ThisClass::OnViewBoundingBoxes)
	ON_UPDATE_COMMAND_UI(ID_VIEW_BOUNDINGBOXES, &ThisClass::OnUpdateViewBoundingBoxes)
	ON_COMMAND(ID_VIEW_SIGHTRANGES, &ThisClass::OnViewSightRanges)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SIGHTRANGES, &ThisClass::OnUpdateViewSightRanges)
	ON_COMMAND(ID_VIEW_WEAPONRANGES, &ThisClass::OnViewWeaponRanges)
	ON_UPDATE_COMMAND_UI(ID_VIEW_WEAPONRANGES, &ThisClass::OnUpdateViewWeaponRanges)
	ON_COMMAND(ID_HIGHLIGHT_TESTART, &ThisClass::OnHighlightTestArt)
	ON_UPDATE_COMMAND_UI(ID_HIGHLIGHT_TESTART, &ThisClass::OnUpdateHighlightTestArt)
	ON_COMMAND(ID_SHOW_LETTERBOX, &ThisClass::OnShowLetterbox)
	ON_UPDATE_COMMAND_UI(ID_SHOW_LETTERBOX, &ThisClass::OnUpdateShowLetterbox)
	ON_COMMAND(ID_VIEW_GARRISONED, &ThisClass::OnViewGarrisoned)
	ON_UPDATE_COMMAND_UI(ID_VIEW_GARRISONED, &ThisClass::OnUpdateViewGarrisoned)
	ON_COMMAND(ID_VIEW_LAYERS_LIST, &ThisClass::OnViewLayersList)
	ON_UPDATE_COMMAND_UI(ID_VIEW_LAYERS_LIST, &ThisClass::OnUpdateViewLayersList)
	ON_COMMAND(ID_VIEW_SHOWMAPBOUNDARIES, &ThisClass::OnViewShowMapBoundaries)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWMAPBOUNDARIES, &ThisClass::OnUpdateViewShowMapBoundaries)
	ON_COMMAND(ID_VIEW_SHOWAMBIENTSOUNDS, &ThisClass::OnViewShowAmbientSounds)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SHOWAMBIENTSOUNDS, &ThisClass::OnUpdateViewShowAmbientSounds)
  ON_COMMAND(ID_VIEW_SHOW_SOUND_CIRCLES, &ThisClass::OnViewShowSoundCircles)
  ON_UPDATE_COMMAND_UI(ID_VIEW_SHOW_SOUND_CIRCLES, &ThisClass::OnUpdateViewShowSoundCircles)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
