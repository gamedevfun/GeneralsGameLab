...
BEGIN_MESSAGE_MAP(BuildList, COptionsPanel)
	...
	ON_CBN_SELCHANGE(IDC_SIDES_COMBO, &ThisClass::OnSelchangeSidesCombo)
	ON_BN_CLICKED(IDC_MOVE_UP, &ThisClass::OnMoveUp)
	ON_BN_CLICKED(IDC_MOVE_DOWN, &ThisClass::OnMoveDown)
	ON_BN_CLICKED(IDC_ADD_BUILDING, &ThisClass::OnAddBuilding)
	ON_LBN_SELCHANGE(IDC_BUILD_LIST, &ThisClass::OnSelchangeBuildList)
	ON_BN_CLICKED(IDC_ALREADY_BUILD, &ThisClass::OnAlreadyBuild)
	ON_BN_CLICKED(IDC_DELETE_BUILDING, &ThisClass::OnDeleteBuilding)
	ON_CBN_SELENDOK(IDC_REBUILDS, &ThisClass::OnSelendokRebuilds)
	ON_CBN_EDITCHANGE(IDC_REBUILDS, &ThisClass::OnEditchangeRebuilds)
	ON_LBN_DBLCLK(IDC_BUILD_LIST, &ThisClass::OnDblclkBuildList)
	ON_EN_CHANGE(IDC_MAPOBJECT_ZOffset, &ThisClass::OnChangeZOffset)
	ON_EN_CHANGE(IDC_MAPOBJECT_Angle, &ThisClass::OnChangeAngle)
	ON_BN_CLICKED(IDC_EXPORT, &ThisClass::OnExport)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
