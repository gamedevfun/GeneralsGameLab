...
BEGIN_MESSAGE_MAP(PlayerListDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_NEWPLAYER, &ThisClass::OnNewplayer)
	ON_BN_CLICKED(IDC_EDITPLAYER, &ThisClass::OnEditplayer)
	ON_BN_CLICKED(IDC_REMOVEPLAYER, &ThisClass::OnRemoveplayer)
	ON_LBN_SELCHANGE(IDC_PLAYERS, &ThisClass::OnSelchangePlayers)
	ON_LBN_DBLCLK(IDC_PLAYERS, &ThisClass::OnDblclkPlayers)
	ON_LBN_SELCHANGE(IDC_ALLIESLIST, &ThisClass::OnSelchangeAllieslist)
	ON_LBN_SELCHANGE(IDC_ENEMIESLIST, &ThisClass::OnSelchangeEnemieslist)
	ON_BN_CLICKED(IDC_PLAYERISCOMPUTER, &ThisClass::OnPlayeriscomputer)
	ON_CBN_SELENDOK(IDC_PLAYERFACTION, &ThisClass::OnEditchangePlayerfaction)
	ON_BN_CLICKED(IDC_CHANGE_NAME, &ThisClass::OnChangePlayername)
	ON_EN_CHANGE(IDC_PLAYERDISPLAYNAME, &ThisClass::OnChangePlayerdisplayname)
	ON_BN_CLICKED(IDC_PlayerColor, &ThisClass::OnColorPress)
	ON_CBN_SELENDOK(IDC_PlayerColorCombo, &ThisClass::OnSelectPlayerColor)
	ON_BN_CLICKED(IDC_ADDSKIRMISHPLAYERS, &ThisClass::OnAddskirmishplayers)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
